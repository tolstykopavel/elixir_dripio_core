defmodule DripioCore.State.AccountConfirmation do
  @moduledoc false
  ...
  def validate(
    ...
    tokens =
      ...
      |> Enum.filter(fn {email, timestamp} ->
        ...
        unless to do
          User.reset_confirmation_token(email)
        end
  ...
  defp get_email_timeout_left(ts) do
    reset_timeout = Application.get_env(:dripio_core, :confirm_email_timeout)
    reset_timeout - (System.monotonic_time(:second) - ts)
  end

  defp get_token_timeout_left(ts) do
    token_lifetime = Application.get_env(:dripio_core, :confirm_token_lifetime)
